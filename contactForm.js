...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (interestUse && interestResell) {
        const interestLabels = document.querySelectorAll('label[for="interest_use"], label[for="interest_resell"]');
...
function handleFormSubmit(e) {
    ...
    const interestUse = document.getElementById('interest_use');
    const interestResell = document.getElementById('interest_resell');
    const interest = interestUse && interestUse.checked ? interestUse.value : (interestResell && interestResell.checked ? interestResell.value : 'Not specified');
...
function showNotification(message, type) {
    ...
    notification.className = `notification fixed bottom-4 right-4 py-4 px-6 rounded-lg shadow-xl z-50 transition-all duration-300 transform translate-y-0 opacity-100 ${type === 'error' ? 'bg-red-600 text-white' : 'bg-green-600 text-white'} flex items-center`;
