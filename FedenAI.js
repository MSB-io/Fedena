class FedenAI {
    ...
    saveChatSessions() {
        try {
            localStorage.setItem('fedenai_chat_sessions', JSON.stringify(this.chatSessions));
        } catch (error) {
    ...
    renderMessage(message) {
        ...
        messageDiv.innerHTML = `
            <div class="max-w-3xl ${message.sender === 'user' ? 'order-2' : 'order-1'}">
                <div class="flex items-start space-x-3 ${message.sender === 'user' ? 'flex-row-reverse space-x-reverse' : ''}">
                    <!-- Avatar -->
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 400) {
                    throw new Error(`Invalid API key or request format. Response: ${errorText}`);
                } else if (response.status === 403) {
                    throw new Error(`API access forbidden. Check your API key permissions. Response: ${errorText}`);
